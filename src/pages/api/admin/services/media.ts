...
/**
 * GET /api/admin/services/media
 * List services media with optional type filter
 */
...
/**
 * POST /api/admin/services/media
 * Upload a file (multipart/form-data) or JSON action (delete, update metadata)
 */
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (contentType.includes("application/json")) {
    ...
    if (action === "delete") {
      ...
      const [existing] = await db.select().from(servicesMedia).where(eq(servicesMedia.id, id));
      if (existing) {
        await deleteStoredUpload(existing.url, "services");

        await db.delete(servicesMedia).where(eq(servicesMedia.id, id));

        await db.insert(auditLog).values({
          id: generateId(),
          action: "services.media.delete",
          userId,
          targetId: id,
          createdAt: new Date(),
        });
    ...
    if (action === "update") {
      ...
      await db.update(servicesMedia).set(updateData).where(eq(servicesMedia.id, id));
  ...
  try {
    ...
    const id = generateId();
    const { filename, url: fileUrl } = await storeImageUpload(file, "services");
    ...
    console.error("[admin/services/media]", error);
